...
public class MixedIndexUpdateApplierTest {
    ...
    @Test
    public void removesStaleDocumentWhenCustomIdVertexIsRecreatedWithDifferentLabel() throws InterruptedException {
        // Custom-id graphs can REUSE a vertex id: a document written for the id's previous incarnation (which matched
        // an indexOnly restriction) must be removed when the applier finds the id now bound to a non-matching label.
        // Otherwise the index would keep returning the old incarnation forever -- in cdc-only mode no future event
        // could ever repair it, because every apply would take the same "index does not apply -> skip" branch.
        ModifiableConfiguration config = GraphDatabaseConfiguration.buildGraphConfiguration();
        config.set(STORAGE_BACKEND, "inmemory");
        config.set(INDEX_BACKEND, "lucene", BACKING);
        config.set(INDEX_DIRECTORY, tempDir.resolve("lucene-relabel").toString(), BACKING);
        config.set(GraphDatabaseConfiguration.INDEX_CDC_ENABLED, true, BACKING);
        config.set(GraphDatabaseConfiguration.INDEX_CDC_SYNCHRONOUS, false, BACKING);
        config.set(GraphDatabaseConfiguration.ALLOW_SETTING_VERTEX_ID, true);
        config.set(GraphDatabaseConfiguration.ALLOW_CUSTOM_VERTEX_ID_TYPES, true);
        JanusGraph customIdGraph = JanusGraphFactory.open(config.getConfiguration());
        try {
            JanusGraphManagement mgmt = customIdGraph.openManagement();
            PropertyKey name = mgmt.makePropertyKey("name").dataType(String.class).make();
            org.janusgraph.core.VertexLabel person = mgmt.makeVertexLabel("person").make();
            mgmt.makeVertexLabel("robot").make();
            mgmt.buildIndex("vsearch", Vertex.class).addKey(name).indexOnly(person).buildMixedIndex(BACKING);
            mgmt.commit();
            ManagementSystem.awaitGraphIndexStatus(customIdGraph, "vsearch").status(SchemaStatus.ENABLED)
                .timeout(60, ChronoUnit.SECONDS).call();
            MixedIndexUpdateApplier customIdApplier =
                new MixedIndexUpdateApplier((StandardJanusGraph) customIdGraph, Collections.singleton(BACKING));

            customIdGraph.addVertex(org.apache.tinkerpop.gremlin.structure.T.id, "x",
                org.apache.tinkerpop.gremlin.structure.T.label, "person", "name", "grace");
            customIdGraph.tx().commit();
            customIdApplier.apply(Collections.singletonList(new CdcElementChange(ElementCategory.VERTEX, "x")));
            assertEquals(1L, customIdGraph.indexQuery("vsearch", "v.name:grace").vertexStream().count(),
                "the person incarnation is indexed");

            customIdGraph.traversal().V("x").drop().iterate();
            customIdGraph.tx().commit();
            customIdGraph.addVertex(org.apache.tinkerpop.gremlin.structure.T.id, "x",
                org.apache.tinkerpop.gremlin.structure.T.label, "robot", "name", "grace");
            customIdGraph.tx().commit();

            customIdApplier.apply(Collections.singletonList(new CdcElementChange(ElementCategory.VERTEX, "x")));
            assertEquals(0L, customIdGraph.indexQuery("vsearch", "v.name:grace").vertexStream().count(),
                "the stale document of the previous (person) incarnation is removed although the live vertex "
                    + "does not match the index restriction");
        } finally {
            customIdGraph.close();
        }
    }
    ...
    @Test
    public void appliesPartitionedVertexAndEdgeChangesUnderTheIdContract() throws InterruptedException {
        // The applier-side half of the partitioned-vertex id contract (the decoder-side half is pinned in
        // DebeziumCassandraJsonDecoderTest): VERTEX changes carry the CANONICAL id and must resolve to the hub's
        // vertex document; EDGE changes carry the raw PARTITION-REPRESENTATIVE endpoint ids exactly as element.id()
        // reports them and must resolve (and remove) the edge document.
        ModifiableConfiguration config = GraphDatabaseConfiguration.buildGraphConfiguration();
        config.set(STORAGE_BACKEND, "inmemory");
        config.set(GraphDatabaseConfiguration.CLUSTER_MAX_PARTITIONS, 8);
        config.set(INDEX_BACKEND, "lucene", BACKING);
        config.set(INDEX_DIRECTORY, tempDir.resolve("lucene-partitioned").toString(), BACKING);
        config.set(GraphDatabaseConfiguration.INDEX_CDC_ENABLED, true, BACKING);
        config.set(GraphDatabaseConfiguration.INDEX_CDC_SYNCHRONOUS, false, BACKING);
        JanusGraph partitionedGraph = JanusGraphFactory.open(config.getConfiguration());
        try {
            ...
            mgmt.buildIndex("esearch", Edge.class).addKey(since).buildMixedIndex(BACKING);
            mgmt.commit();
            ManagementSystem.awaitGraphIndexStatus(partitionedGraph, "vsearch").status(SchemaStatus.ENABLED)
                .timeout(60, ChronoUnit.SECONDS).call();
            ManagementSystem.awaitGraphIndexStatus(partitionedGraph, "esearch").status(SchemaStatus.ENABLED)
                .timeout(60, ChronoUnit.SECONDS).call();
            MixedIndexUpdateApplier partitionedApplier =
                new MixedIndexUpdateApplier((StandardJanusGraph) partitionedGraph, Collections.singleton(BACKING));

            JanusGraphVertex hub = partitionedGraph.addVertex(
                org.apache.tinkerpop.gremlin.structure.T.label, "hub", "name", "hubble");
            JanusGraphVertex other = partitionedGraph.addVertex();
            Edge e = hub.addEdge("knows", other, "since", "2044");
            partitionedGraph.tx().commit();
            Long hubId = (Long) hub.id();                         // canonical id (VERTEX-change contract)
            RelationIdentifier eid = (RelationIdentifier) e.id(); // raw representative endpoints (EDGE-change contract)

            partitionedApplier.apply(Arrays.asList(new CdcElementChange(ElementCategory.VERTEX, hubId),
                new CdcElementChange(ElementCategory.EDGE, eid)));
            assertEquals(1L, partitionedGraph.indexQuery("vsearch", "v.name:hubble").vertexStream().count(),
                "the partitioned hub's vertex document resolves via its canonical id");
            assertEquals(1L, partitionedGraph.indexQuery("esearch", "e.since:2044").edgeStream().count(),
                "the hub's edge document resolves via its raw partition-representative RelationIdentifier");

            partitionedGraph.traversal().V(hubId).property("name", "hubbles").iterate();
            partitionedGraph.tx().commit();
            partitionedApplier.apply(Collections.singletonList(new CdcElementChange(ElementCategory.VERTEX, hubId)));
            assertEquals(0L, partitionedGraph.indexQuery("vsearch", "v.name:hubble").vertexStream().count());
            assertEquals(1L, partitionedGraph.indexQuery("vsearch", "v.name:hubbles").vertexStream().count(),
                "updates to the partitioned vertex converge under the canonical id");

            partitionedGraph.traversal().E(eid).drop().iterate();
            partitionedGraph.tx().commit();
            partitionedApplier.apply(Collections.singletonList(new CdcElementChange(ElementCategory.EDGE, eid)));
            assertEquals(0L, partitionedGraph.indexQuery("esearch", "e.since:2044").edgeStream().count(),
                "the removed edge's document is deleted via its raw representative identity");
        } finally {
