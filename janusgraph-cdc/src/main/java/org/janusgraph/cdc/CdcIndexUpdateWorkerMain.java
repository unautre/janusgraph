...
public final class CdcIndexUpdateWorkerMain implements AutoCloseable {
    ...
    /** Whether at least one worker thread is still running. {@code false} before {@link #start()}, after
     *  {@link #close()}, and -- the case embedders must watch for -- after every worker thread died from an
     *  unexpected {@link Error} (see {@link CdcIndexUpdateWorker#isAlive()}): the process then looks healthy while
     *  consuming nothing and the index silently goes stale. {@link #main} supervises this and exits. */
    public boolean hasAliveWorkers() {
        for (CdcIndexUpdateWorker worker : workers) {
            if (worker.isAlive()) {
                return true;
            }
        }
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Keep the process alive until a shutdown signal (Ctrl-C / SIGTERM) fires the hook above -- while
        // supervising worker liveness: a worker thread killed by an Error (OutOfMemoryError, NoClassDefFoundError,
        // ...) is only logged by its uncaught-exception handler, and once ALL workers are dead the process would
        // otherwise keep running as a healthy-looking zombie that consumes nothing while the index goes stale.
        // Exit non-zero instead so a process supervisor (systemd, Kubernetes, ...) restarts the worker; the exit
        // runs the shutdown hook above, which closes the graph (offsets of any in-flight batch were never
        // committed, so Kafka redelivers it after the restart -- at-least-once holds).
        // (The shutdownRequested guard keeps a CLEAN shutdown from tripping this check: the hook stops the workers
        // before its graph.close() finishes and counts the latch down, so an await tick landing in that window would
        // otherwise see zero live workers and log a spurious "died unexpectedly" error.)
        while (!shutdownLatch.await(10, TimeUnit.SECONDS)) {
            if (!shutdownRequested.get() && !runner.hasAliveWorkers()) {
                log.error("All {} CDC index-update worker(s) have died unexpectedly; exiting so the process "
                    + "supervisor can restart this worker process", runner.getWorkerCount());
                System.exit(1);
            }
        }
    }
