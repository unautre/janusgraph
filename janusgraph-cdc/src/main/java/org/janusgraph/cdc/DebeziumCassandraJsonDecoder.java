...
public class DebeziumCassandraJsonDecoder implements CdcEventDecoder {
    ...
    /** Whether the graph permits String vertex ids ({@code graph.allow-custom-vid-types}). When it does not, no
     *  legitimate edgestore partition key can decode to a String, so such a key marks a corrupt record. */
    private final boolean allowStringVertexIds;
    private final ObjectMapper mapper = new ObjectMapper();

    public DebeziumCassandraJsonDecoder(StandardJanusGraph graph) {
        this.graph = graph;
        this.idManager = graph.getIDManager();
        this.allowStringVertexIds = graph.getConfiguration().allowCustomVertexIdType();
    }
    ...
    public Collection<CdcElementChange> decode(byte[] messageKey, byte[] messageValue) {
        ...
        if (keyBytes == null || keyBytes == MALFORMED_CELL) {
            if (keyBytes == null) {
                // Every row-level Debezium event carries the partition key; a missing one is an unexpected envelope
                // shape, and silently dropping it would hide e.g. a connector format change. (The malformed case has
                // already been logged by decodeCell.)
                log.warn("Skipping CDC record without a decodable partition key (unexpected event shape)");
            }
            return Collections.emptyList();
        }
        ...
        if (vertexId instanceof String && !allowStringVertexIds) {
            // getKeyID decodes any non-long-sized key as a String id -- but on a graph whose id regime forbids
            // string vertex ids no legitimate edgestore key has that shape, so this is a corrupt (or hostile)
            // record. Skip it like the other poison-pill shapes above; letting it through would push a
            // garbage-keyed change into the applier (a wasted index-backend delete under an arbitrary document id,
            // and one degraded batched preload per delivery).
            log.warn("Skipping CDC record: partition key decodes to a string vertex id, but custom (string) vertex "
                + "ids are disabled for this graph -- the key cannot belong to any vertex");
            return Collections.emptyList();
        }
        ...
        if (columnBytes == MALFORMED_CELL) {
            // The column is PRESENT but undecodable (logged by decodeCell): degrade to a vertex reindex -- the
            // vertex documents stay correct -- but never silently, since the relation-document part of this event
            // is lost.
            log.warn("Falling back to reindexing only vertex {} for a CDC record whose clustering column could "
                + "not be decoded", vertexId);
            return Collections.singletonList(vertexChange(vertexId));
        }
        byte[] valueBytes = decodeCell(data, "value");
        if (valueBytes == MALFORMED_CELL) {
            valueBytes = null; // logged by decodeCell; the header-only relation parse below works without the value
        }
        return resolveChanges(vertexId, columnBytes, valueBytes);
    }
    ...
    private Collection<CdcElementChange> resolveChanges(Object vertexId, byte[] columnBytes, byte[] valueBytes) {
        ...
        try {
            ...
            // Defensive: an edge column whose header PARSED but yielded no recoverable relation id. In practice
            // constrained-multiplicity edge delete tombstones do not land here -- their relation id lives in the
            // value region, so the header parse throws reading the empty value and they take the catch fallback
            // below instead. Should corrupt-but-parseable data reach this branch, reindex whatever endpoints are
            // known from current state; edge DOCUMENT deletions never depend on this event anyway (they are written
            // synchronously at commit even in cdc-only mode, exactly because tombstones cannot identify them).
            final List<CdcElementChange> changes = new ArrayList<>(2);
            changes.add(vertexChange(vertexId));
            ...
            // Columns whose relation header cannot be parsed fall back to reindexing the owning vertex from current
            // state, which keeps all vertex-element indexes correct. This is where value-less delete tombstones of
            // PROPERTIES and of CONSTRAINED-multiplicity edges land (both store identity in the value region, so the
            // parse throws reading the empty value). Nothing is lost: their relation-element DOCUMENTS are deleted
            // synchronously at commit even in cdc-only mode, precisely because such tombstones cannot identify them.
            // A dropped relation type also lands here (its schema lookup fails without a backend cause), converging
            // the same way.
    ...
    /** Identity marker distinguishing "cell present but not decodable" from a legitimately absent/null cell
     *  ({@code null}). Compared by identity: a genuinely empty blob decodes to a DIFFERENT empty array instance. */
    private static final byte[] MALFORMED_CELL = new byte[0];
     ...
     *
     * <p>Absent and malformed cells are deliberately distinct results: an absent {@code column1} legitimately means
     * a partition-level mutation, while a PRESENT cell of an unexpected shape means the record (or the connector's
     * envelope format) is broken -- silently conflating the two would degrade every relation event to a vertex-only
     * reindex with no trace in the logs.</p>
     *
     * @return the decoded bytes; {@code null} if the column is absent or JSON-null; {@link #MALFORMED_CELL} (by
     *         identity) if the cell is present but cannot be decoded -- a warning has been logged in that case
     */
    private byte[] decodeCell(JsonNode after, String columnName) {
        ...
        if (valueNode == null || valueNode.isNull()) {
            return null; // {"value": null, "deletion_ts": ...}: a null cell, e.g. the deleted side of a tombstone
        }
        if (!valueNode.isTextual()) {
            log.warn("CDC record column '{}' has an unexpected shape (expected a Base64 string, got {}); "
                + "was the Debezium envelope format changed?", columnName, valueNode.getNodeType());
            return MALFORMED_CELL;
        }
        try {
            return Base64.getDecoder().decode(valueNode.asText());
        } catch (IllegalArgumentException e) {
            // A corrupt / poison-pill record (non-Base64 blob). Mark it malformed rather than let the exception
            // propagate and make the worker rewind-and-retry the same undecodable record forever (no forward progress).
            log.warn("CDC record column '{}' is not valid Base64", columnName, e);
            return MALFORMED_CELL;
        }
    }
}
