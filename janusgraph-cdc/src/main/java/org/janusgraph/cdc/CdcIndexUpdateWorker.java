...
public class CdcIndexUpdateWorker implements Runnable, AutoCloseable {
        ...
        try {
            ...
            while (running && !Thread.currentThread().isInterrupted()) {
                try {
                    ...
                    // This path only runs once the batch's whole retry budget is already spent (or decoding hit a
                    // backend failure, which has no inner retries at all), so pause a full second before the next
                    // poll/decode/apply cycle: pacing the error loop off the raw retry.initial-wait would let a
                    // legitimate 0ms "retry immediately" tuning hot-spin here, flooding the log and the backend.
                    sleepQuietly(Math.max(config.getRetryInitialWait().toMillis(), 1000L));
                }
            }
        } finally {
        ...
        while (true) {
            try {
                ...
                if (shutdownInitiated || Thread.currentThread().isInterrupted()) {
                    // A shutdown arrived during the backoff sleep: rethrow (so pollOnce() rewinds and the batch is
                    // redelivered) instead of running one more apply attempt against a closing graph -- that
                    // attempt could block up to the full index/storage timeout and outlive awaitTermination().
                    throw e;
                }
            }
        }
    ...
    private long backoffMillis(int attempt) {
        ...
        // Jitter into [wait/2, wait]: workers sharing a struggling backend otherwise retry in lockstep (they all
        // received their batches, failed, and slept the same deterministic schedule), hammering it simultaneously.
        return wait <= 1 ? wait : (wait / 2) + ThreadLocalRandom.current().nextLong(wait - (wait / 2) + 1);
    }
        ...
        for (TopicPartition tp : records.partitions()) {
            ...
            if (!partitionRecords.isEmpty()) {
                try {
                    ...
                    // Most likely the partition was revoked since the poll (possible without an intervening poll
                    // under the KIP-848 consumer protocol, reachable via cdc.consumer.group.protocol). Safe to skip:
                    // its offsets were never committed, so the new owner redelivers from the committed position.
                    // Crucially, keep rewinding the REMAINING partitions -- aborting the loop would let a later
                    // commit advance past their unapplied records.
                    log.warn("Could not rewind partition {} (revoked?); its records will be redelivered by the "
                        + "group instead", tp, e);
                }
            }
        }
